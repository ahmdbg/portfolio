...
// Add WhatsApp form handling
form.addEventListener('submit', (e) => {
    e.preventDefault();
    
    const fullname = form.querySelector('[name="fullname"]').value;
    const phone = form.querySelector('[name="phone"]').value;
    const message = form.querySelector('[name="text"]').value;
    
    // Format the WhatsApp message
    const text = `Name: ${fullname}${phone ? '\nPhone: ' + phone : ''}\nMessage: ${message}`;
    
    // Create WhatsApp URL with encoded message
    const whatsappUrl = `https://wa.me/6285640054840?text=${encodeURIComponent(text)}`;
...
function initSkillsAnimation() {
  ...
  function animateSkills() {
    skillItems.forEach(item => {
      ...
      const targetValue = parseInt(valueElement.value);
      
      // Set width based on the data value
  ...
  // Also trigger animation when resume tab is clicked
  const resumeNav = document.querySelector('[data-nav-link="resume"]');
  if (resumeNav) {
    resumeNav.addEventListener('click', () => {
      setTimeout(animateSkills, 300); // Small delay for tab transition
    });
  }
}
